    ...
    class TGM_Plugin_Activation {
        ...
        public function init() {
            ...
            $this->strings = array(
                ...
                'installing'                      => __( 'Installing Plugin: %s', 'mythemeshop' ),
                ...
                'dashboard'                       => __( 'Return to the dashboard', 'mythemeshop' ),
                'plugin_activated'                => __( 'Plugin activated successfully.', 'mythemeshop' ),
                'activated_successfully'          => __( 'The following plugin was activated successfully:', 'mythemeshop' ),
                'plugin_already_active'           => __( 'No action taken. Plugin %1$s was already active.', 'mythemeshop' ),
                ...
                'complete'                        => __( 'All plugins installed and activated successfully. %1$s', 'mythemeshop' ),
                'dismiss'                         => __( 'Dismiss this notice', 'mythemeshop' ),
                'contact_admin'                   => __( 'Please contact the administrator of this site for help.', 'mythemeshop' ),
            );
        ...
        public function filter_plugin_action_links_update( $actions ) {
            $actions['update'] = sprintf(
                ...
                esc_attr__( 'This plugin needs to be updated to be compatible with your theme.', 'mythemeshop' ),
                esc_html__( 'Update Required', 'mythemeshop' )
            );
        ...
        protected function add_admin_menu( array $args ) {
            if ( has_filter( 'tgmpa_admin_menu_use_add_theme_page' ) ) {
                _deprecated_function( 'The "tgmpa_admin_menu_use_add_theme_page" filter', '2.5.0', esc_html__( 'Set the parent_slug config variable instead.', 'mythemeshop' ) );
            }
        ...
        public function do_plugin_install() {
            ...
            if ( ( isset( $_GET['tgmpa-install'] ) && 'install-plugin' === $_GET['tgmpa-install'] ) || ( isset( $_GET['tgmpa-update'] ) && 'update-plugin' === $_GET['tgmpa-update'] ) ) {
                ...
                if ( $this->is_tgmpa_complete() ) {
                    echo '<p>', sprintf( esc_html( $this->strings['complete'] ), '<a href="' . esc_url( self_admin_url() ) . '">' . esc_html__( 'Return to the Dashboard', 'mythemeshop' ) . '</a>' ), '</p>';
            ...
            if ( ! empty( $desired_slug ) ) {
                ...
                if ( ! empty( $subdir_name ) && $subdir_name !== $desired_slug ) {
                    ...
                    if ( true === $GLOBALS['wp_filesystem']->move( $from, $to ) ) {
                        ...
                        return new WP_Error( 'rename_failed', esc_html__( 'The remote plugin package does not contain a folder with the desired slug and renaming did not work.', 'mythemeshop' ) . ' ' . esc_html__( 'Please contact the plugin provider and ask them to package their plugin according to the WordPress guidelines.', 'mythemeshop' ), array( 'found' => $subdir_name, 'expected' => $desired_slug ) );
                    }
        ...
        public function notices() {
            ...
            if ( ! empty( $message ) ) {
                ...
                foreach ( $message as $type => $plugin_group ) {
                    ...
                    $imploded       = empty( $linked_plugins ) ? $last_plugin : ( implode( ', ', $linked_plugins ) . ' ' . esc_html_x( 'and', 'plugin A *and* plugin B', 'mythemeshop' ) . ' ' . $last_plugin );

                    $rendered .= sprintf(
                        $line_template,
                        sprintf(
                            translate_nooped_plural( $this->strings[ $type ], $count, 'mythemeshop' ),
                            $imploded,
                            $count
                        )
                    );
                ...
                if ( current_user_can( 'install_plugins' ) ) {
                    if ( $install_link_count > 0 ) {
                        $action_links['install'] = sprintf(
                            $link_template,
                            translate_nooped_plural( $this->strings['install_link'], $install_link_count, 'mythemeshop' ),
                    ...
                    if ( $update_link_count > 0 ) {
                        $action_links['update'] = sprintf(
                            $link_template,
                            translate_nooped_plural( $this->strings['update_link'], $update_link_count, 'mythemeshop' ),
                            esc_url( $this->get_tgmpa_status_url( 'update' ) )
                        );
                    }
                }
                ...
                if ( current_user_can( 'activate_plugins' ) && $activate_link_count > 0 ) {
                    $action_links['activate'] = sprintf(
                        $link_template,
                        translate_nooped_plural( $this->strings['activate_link'], $activate_link_count, 'mythemeshop' ),
    ...
    $("#setting-error-tgmpa").on("click", ".notice-dismiss", function(e) {
        ...
        public function register( $plugin ) {
            ...
            if ( !empty ( $plugin['available_separately'] ) ) {
                $item['source'] = __( 'Available Separately', 'mythemeshop' );
            }
...
if ( ! class_exists( 'TGMPA_List_Table' ) ) {
    ...
    class TGMPA_List_Table extends WP_List_Table {
        ...
        protected function get_plugin_advise_type_text( $required ) {
            if ( true === $required ) {
                return __( 'Required', 'mythemeshop' );
            }

            return __( 'Recommended', 'mythemeshop' );
        }
        ...
        protected function get_plugin_source_type_text( $type ) {
            ...
            switch ( $type ) {
                case 'repo':
                    $string = __( 'WordPress Repository', 'mythemeshop' );
                    break;
                case 'external':
                    $string = __( 'External Source', 'mythemeshop' );
                    break;
                case 'bundled':
                    $string = __( 'Pre-Packaged', 'mythemeshop' );
                    break;
            }
        ...
        protected function get_plugin_status_text( $slug ) {
            if ( ! $this->tgmpa->is_plugin_installed( $slug ) ) {
                return __( 'Not Installed', 'mythemeshop' );
            }

            if ( ! $this->tgmpa->is_plugin_active( $slug ) ) {
                $install_status = __( 'Installed But Not Activated', 'mythemeshop' );
            } else {
                $install_status = __( 'Active', 'mythemeshop' );
            }

            $update_status = '';

            if ( $this->tgmpa->does_plugin_require_update( $slug ) && false === $this->tgmpa->does_plugin_have_update( $slug ) ) {
                $update_status = __( 'Required Update not Available', 'mythemeshop' );

            } elseif ( $this->tgmpa->does_plugin_require_update( $slug ) ) {
                $update_status = __( 'Requires Update', 'mythemeshop' );

            } elseif ( false !== $this->tgmpa->does_plugin_have_update( $slug ) ) {
                $update_status = __( 'Update recommended', 'mythemeshop' );
            }
            ...
            return sprintf(
                _x( '%1$s, %2$s', '%1$s = install status, %2$s = update status', 'mythemeshop' ),
                $install_status,
                $update_status
            );
        }
        ...
        public function get_views() {
            ...
            foreach ( $this->view_totals as $type => $count ) {
                ...
                switch ( $type ) {
                    case 'all':
                        $text = wp_kses( _nx( 'All <span class="count">(%s)</span>', 'All <span class="count">(%s)</span>', $count, 'plugins', 'mythemeshop' ), array( 'span' ) );
                        break;
                    case 'install':
                        $text = wp_kses( _n( 'To Install <span class="count">(%s)</span>', 'To Install <span class="count">(%s)</span>', $count, 'mythemeshop' ), array( 'span' ) );
                        break;
                    case 'update':
                        $text = wp_kses( _n( 'Update Available <span class="count">(%s)</span>', 'Update Available <span class="count">(%s)</span>', $count, 'mythemeshop' ), array( 'span' ) );
                        break;
                    case 'activate':
                        $text = wp_kses( _n( 'To Activate <span class="count">(%s)</span>', 'To Activate <span class="count">(%s)</span>', $count, 'mythemeshop' ), array( 'span' ) );
                        break;
                    default:
                        $text = '';
                        break;
        ...
        public function column_description( $item )
        {
            ...
            $installed = ! empty( $item['installed_version'] ) ? $item['installed_version'] : __( 'Not Installed', 'mythemeshop' );
            $description .= sprintf( __( 'Version: %s', 'mythemeshop' ), $installed );
            $description .= ' | ' . sprintf( __( 'Source: %s', 'mythemeshop' ), $item['source'] );

            if ( ! empty( $item['info_link'] ) ) {
                $description .= ' | ' . sprintf( '<a target="_blank" href="%s">' . __( 'Visit plugin site', 'mythemeshop' ) . '</a>', $item['info_link'] );
            }
        ...
        public function column_version( $item ) {
            ...
            if ( $this->tgmpa->is_plugin_installed( $item['slug'] ) ) {
                $installed = ! empty( $item['installed_version'] ) ? $item['installed_version'] : _x( 'unknown', 'as in: "version nr unknown"', 'mythemeshop' );
                ...
                $output[] = sprintf(
                    '<p><span style="min-width: 32px; text-align: right; float: right;%1$s">%2$s</span>' . __( 'Installed version:', 'mythemeshop' ) . '</p>',
                    $color,
                    $installed
                );
            }
            ...
            if ( ! empty( $item['minimum_version'] ) ) {
                $output[] = sprintf(
                    '<p><span style="min-width: 32px; text-align: right; float: right;">%1$s</span>' . __( 'Minimum required version:', 'mythemeshop' ) . '</p>',
            ...
            if ( ! empty( $item['available_version'] ) ) {
                ...
                $output[] = sprintf(
                    '<p><span style="min-width: 32px; text-align: right; float: right;%1$s">%2$s</span>' . __( 'Available version:', 'mythemeshop' ) . '</p>',
                    $color,
        ...
        public function no_items() {
            printf( wp_kses( __( 'No plugins to install, update or activate. <a href="%1$s">Return to the Dashboard</a>', 'mythemeshop' ), array( 'a' => array( 'href' => array() ) ) ), esc_url( self_admin_url() ) );
            echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
        }
        ...
        public function get_columns() {
            $columns = array(
                ...
                'plugin' => __( 'Plugin', 'mythemeshop' ),
                'description' => __( 'Description', 'mythemeshop' ),
            );
        ...
        protected function get_row_actions( $item ) {
            ...
            if ( ! $this->tgmpa->is_plugin_installed( $item['slug'] ) ) {
                $actions['install'] = _x( 'Install %2$s', '%2$s = plugin name in screen reader markup', 'mythemeshop' );
            } else {
                // Display the 'Update' action link if an update is available and WP complies with plugin minimum.
                if ( false !== $this->tgmpa->does_plugin_have_update( $item['slug'] ) && $this->tgmpa->can_plugin_update( $item['slug'] ) ) {
                    $actions['update'] = _x( 'Update %2$s', '%2$s = plugin name in screen reader markup', 'mythemeshop' );
                }

                // Display the 'Activate' action link, but only if the plugin meets the minimum version.
                if ( $this->tgmpa->can_plugin_activate( $item['slug'] ) ) {
                    $actions['activate'] = _x( 'Activate %2$s', '%2$s = plugin name in screen reader markup', 'mythemeshop' );
                }
            }
        ...
        public function wp_plugin_update_row( $slug, $item ) {
            ...
            esc_html__( 'Upgrade message from the plugin author:', 'mythemeshop' ),
        ...
        public function get_bulk_actions() {
            ...
            if ( 'update' !== $this->view_context && 'activate' !== $this->view_context ) {
                if ( current_user_can( 'install_plugins' ) ) {
                    $actions['tgmpa-bulk-install'] = __( 'Install', 'mythemeshop' );
                }
            }

            if ( 'install' !== $this->view_context ) {
                if ( current_user_can( 'update_plugins' ) ) {
                    $actions['tgmpa-bulk-update'] = __( 'Update', 'mythemeshop' );
                }
                if ( current_user_can( 'activate_plugins' ) ) {
                    $actions['tgmpa-bulk-activate'] = __( 'Activate', 'mythemeshop' );
                }
            }
        ...
        public function process_bulk_actions() {
            ...
            if ( 'tgmpa-bulk-install' === $this->current_action() || 'tgmpa-bulk-update' === $this->current_action() ) {
                ...
                if ( empty( $_POST['plugin'] ) ) {
                    if ( 'install' === $install_type ) {
                        $message = __( 'No plugins were selected to be installed. No action taken.', 'mythemeshop' );
                    } else {
                        $message = __( 'No plugins were selected to be updated. No action taken.', 'mythemeshop' );
                    }
                ...
                if ( empty( $plugins_to_install ) ) {
                    if ( 'install' === $install_type ) {
                        $message = __( 'No plugins are available to be installed at this time.', 'mythemeshop' );
                    } else {
                        $message = __( 'No plugins are available to be updated at this time.', 'mythemeshop' );
                    }
            ...
            if ( 'tgmpa-bulk-activate' === $this->current_action() ) {
                ...
                if ( empty( $_POST['plugin'] ) ) {
                    echo '<div id="message" class="error"><p>', esc_html__( 'No plugins were selected to be activated. No action taken.', 'mythemeshop' ), '</p></div>';
                ...
                if ( empty( $plugins_to_activate ) ) {
                    echo '<div id="message" class="error"><p>', esc_html__( 'No plugins are available to be activated at this time.', 'mythemeshop' ), '</p></div>';
                ...
                if ( is_wp_error( $activate ) ) {
                    ...
                    $imploded     = empty( $plugin_names ) ? $last_plugin : ( implode( ', ', $plugin_names ) . ' ' . esc_html_x( 'and', 'plugin A *and* plugin B', 'mythemeshop' ) . ' ' . $last_plugin );

                    printf( // WPCS: xss ok.
                        '<div id="message" class="updated"><p>%1$s %2$s.</p></div>',
                        esc_html( _n( 'The following plugin was activated successfully:', 'The following plugins were activated successfully:', $count, 'mythemeshop' ) ),
                        $imploded
                    );
            ...
            if ( ! class_exists( 'TGMPA_Bulk_Installer' ) ) {
                ...
                class TGMPA_Bulk_Installer extends Plugin_Upgrader {
                    ...
                    public function activate_strings() {
                        $this->strings['activation_failed']  = __( 'Plugin activation failed.', 'mythemeshop' );
                        $this->strings['activation_success'] = __( 'Plugin activated successfully.', 'mythemeshop' );
                    }
                ...
                class TGMPA_Bulk_Installer_Skin extends Bulk_Upgrader_Skin {
                    ...
                    public function add_strings() {
                        if ( 'update' === $this->options['install_type'] ) {
                            parent::add_strings();
                            $this->upgrader->strings['skin_before_update_header'] = __( 'Updating Plugin %1$s (%2$d/%3$d)', 'mythemeshop' );
                        } else {
                            $this->upgrader->strings['skin_update_failed_error'] = wp_kses( __( 'An error occurred while installing %1$s: <strong>%2$s</strong>.', 'mythemeshop' ), array( 'strong' ) );
                            $this->upgrader->strings['skin_update_failed']       = __( 'The installation of %1$s failed.', 'mythemeshop' );

                            if ( $this->tgmpa->is_automatic ) {
                                // Automatic activation strings.
                                $this->upgrader->strings['skin_upgrade_start']        = __( 'The installation and activation process is starting. This process may take a while on some hosts, so please be patient.', 'mythemeshop' );
                                $this->upgrader->strings['skin_update_successful']    = __( '%1$s installed and activated successfully.', 'mythemeshop' ) . ' <a href="#" class="hide-if-no-js" onclick="%2$s"><span>' . esc_html__( 'Show Details', 'mythemeshop' ) . '</span><span class="hidden">' . esc_html__( 'Hide Details', 'mythemeshop' ) . '</span>.</a>';
                                $this->upgrader->strings['skin_upgrade_end']          = __( 'All installations and activations have been completed.', 'mythemeshop' );
                                $this->upgrader->strings['skin_before_update_header'] = __( 'Installing and Activating Plugin %1$s (%2$d/%3$d)', 'mythemeshop' );
                            } else {
                                // Default installation strings.
                                $this->upgrader->strings['skin_upgrade_start']        = __( 'The installation process is starting. This process may take a while on some hosts, so please be patient.', 'mythemeshop' );
                                $this->upgrader->strings['skin_update_successful']    = esc_html__( '%1$s installed successfully.', 'mythemeshop' ) . ' <a href="#" class="hide-if-no-js" onclick="%2$s"><span>' . esc_html__( 'Show Details', 'mythemeshop' ) . '</span><span class="hidden">' . esc_html__( 'Hide Details', 'mythemeshop' ) . '</span>.</a>';
                                $this->upgrader->strings['skin_upgrade_end']          = __( 'All installations have been completed.', 'mythemeshop' );
                                $this->upgrader->strings['skin_before_update_header'] = __( 'Installing Plugin %1$s (%2$d/%3$d)', 'mythemeshop' );
                            }
                        }
                    }
                    ...
                    public function bulk_footer() {
                        ...
                        if ( $this->tgmpa->is_tgmpa_complete() ) {
                            ...
                            $update_actions['dashboard'] = sprintf(
                                ...
                                '<a href="' . esc_url( self_admin_url() ) . '">' . esc_html__( 'Return to the Dashboard', 'mythemeshop' ) . '</a>'
                            );
                        } else {
...
function mts_register_required_plugins() {
    ...
    $plugins = array(
        ...
        array(
            ...
            'alt_description' => __( 'Create reviews! Choose from Stars, Percentages, Circles or Points for review scores. Supports Retina Display, WPMU & Unlimited Color Schemes.', 'mythemeshop' ),
...
function mts_tgmpba_table_data_item( $item, $plugin ) {
    if ( ! empty ( $plugin['available_separately'] ) ) {
        $item['source'] = __( 'MyThemeShop', 'mythemeshop' );
...
function mts_addons_add_admin_menu_item() {
    add_menu_page(
        __('Install Plugins', 'mythemeshop' ),
        __('Install Plugins', 'mythemeshop' ),
        'manage_options',
        ...
        <form id="tgmpa-plugins" action="" method="post">
            ...
            <div class="wrap mts-addons">
                ...
                <header class="mts-addons-header">
                    <h2><?php _e( 'Recommended Plugins', 'mythemeshop' ); ?></h2>
                    <p><?php printf( __( 'MyThemeShop and third party plugins recommended for %s theme!', 'mythemeshop' ), '<strong>'.$theme_name.'</strong>' ); ?></p>
                </header>
                ...
                if ( isset( $_GET['mts_plugins_list'] ) ) {
                    ...
                    foreach ( $plugins as $plugin ) :
                        ...
                        if ( isset( $plugin['alt'] ) ) {
                            ...
                            // Free
                                ...
                                if ( $tgma_instance->is_plugin_installed( $plugin['alt'] ) ) {
                                    ...
                                    $go_pro_button = '<a class="mts-addon-gopro-button button button-primary" href="' . esc_url( $plugin['info_link'] ) . '" target="_blank">' . __( 'Get Pro Version!', 'mythemeshop' ) . '</a>';
                                }
                            }
                        }
                        ...
                        if ( $tgma_instance->is_plugin_installed( $plugin['slug'] ) ) {
                            ...
                            if ( is_plugin_active( $plugin['file_path'] ) ) {
                                ...
                                $status         = 'active';
                                $status_message = __( 'Active', 'mythemeshop' );
                                $url            = admin_url( 'plugins.php' );
                                $button = '<a class="mts-addon-button button" href="' . esc_url( $url ) . '">'.__('Manage Plugin', 'mythemeshop' ).'</a>';

                            } else {

                                $status         = 'inactive';
                                $status_message = __( 'Inactive', 'mythemeshop' );
                                $url = wp_nonce_url(
                                    add_query_arg(
                                        array(
                                ...
                                $text   = __( 'Activate Plugin', 'mythemeshop' );
                            ...
                            if ( !empty( $plugin['available_separately'] ) || isset( $plugin['pro'] ) ) {
                                ...
                                $text   = __( 'Install Plugin', 'mythemeshop' );
                                $target = '';
                            }
                           
                            $class = 'mts-addon-button button button-primary';
                            $status         = 'not-installed';
                            $status_message = __( 'Not Installed', 'mythemeshop' );
                    ...
                    <ul class="mts-addons-list loading" id="mts-addons-list">
                        <li class="loader"><?php _e( 'Loading...', 'mythemeshop' ); ?></li>
                    </ul>
                    <?php
                } 
                ?>
